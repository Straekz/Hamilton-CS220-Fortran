    ...
    do j = 1, size(matrix, 2) ! Iterate through each column 
        do i = (1 + lead), size(matrix,1) ! Iterate from the row of the last obtained leading one to the max # rows of matrix            
            if ((reducedRow_echelon(i,j) /= 0) .and. (i > lead)) then ! Checks for the first non-zero value in a column
                ...
                ! Makes it so that the row with the leading value is the only non-zero value by subtracting each row from the row with the leading value
                do ii = 1, size(matrix, 1)
                    if ((reducedRow_echelon(ii, j) /= 0) .and. (lead /= ii)) then
                        div =  reducedRow_echelon(ii, j) / reducedRow_echelon(lead, j) ! Divides the row to subtract by the leading value from the same column
                        reducedRow_echelon(ii, :) = reducedRow_echelon(ii, :) - (reducedRow_echelon(lead, :) * div) ! Zeros-out column by subtracting the current row by the row with the leading value
                    end if
                end do
                
                ! Divides everything in the row with the leading value such that the leading value becomes a leading one
